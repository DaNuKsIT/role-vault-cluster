...
function vault_to_network_address {
  ...
  case "$vault_node_name" in
    vault_1)
      echo "http://vault-0.moevideo.net:8200"
      ;;
    vault_2)
      echo "http://vault-1.moevideo.net:8200"
      ;;
    vault_3)
      echo "http://vault-2.moevideo.net:8200"
      ;;
    vault_4)
      echo "http://vault-3.moevideo.net:8200"
      ;;
  esac
}
...
function vault_1 {
    (export VAULT_ADDR=http://vault-0.moevideo.net:8200 && vault "$@")
}

# Create a helper function to address the second vault node
function vault_2 {
    (export VAULT_ADDR=http://vault-1.moevideo.net:8200 && vault "$@")
}

# Create a helper function to address the third vault node
function vault_3 {
    (export VAULT_ADDR=http://vault-2.moevideo.net:8200 && vault "$@")
}

# Create a helper function to address the fourth vault node
function vault_4 {
    (export VAULT_ADDR=http://vault-3.moevideo.net:8200 && vault "$@")
}

function stop_vault {
  local vault_node_name=$1

  service_count=$(pgrep -f /etc/vault.d/config-"$vault_node_name" | wc -l | tr -d '[:space:]')
...
function clean {
  ...
  for loopback_address in "vault-1.moevideo.net" "vault-2.moevideo.net" "vault-3.moevideo.net" ; do
...
function status {
  service_count=$(pgrep -f /etc/vault.d/config | wc -l | tr -d '[:space:]')
...
function create_network {
  ...
  case "$os_name" in
    darwin)
      printf "\n%s" \
      "[vault_2] Enabling local loopback on vault-1.moevideo.net (requires sudo)" \
      ""

      sudo ifconfig lo0 alias vault-1.moevideo.net

      printf "\n%s" \
        "[vault_3] Enabling local loopback on vault-2.moevideo.net (requires sudo)" \
        ""

      sudo ifconfig lo0 alias vault-2.moevideo.net

      printf "\n%s" \
        "[vault_4] Enabling local loopback on vault-3.moevideo.net (requires sudo)" \
        ""

      sudo ifconfig lo0 alias vault-3.moevideo.net
      ;;
    linux)
      printf "\n%s" \
      "[vault_2] Enabling local loopback on vault-1.moevideo.net (requires sudo)" \
      ...
      sudo ip addr add vault-1.moevideo.net/8 dev lo label lo:0

      printf "\n%s" \
        "[vault_3] Enabling local loopback on vault-2.moevideo.net (requires sudo)" \
        ""

      sudo ip addr add vault-2.moevideo.net/8 dev lo label lo:1

      printf "\n%s" \
        "[vault_4] Enabling local loopback on vault-3.moevideo.net (requires sudo)" \
        ""

      sudo ip addr add vault-3.moevideo.net/8 dev lo label lo:2
...
listener "tcp" {
   address = "vault-0.moevideo.net:8200"
   tls_disable = true
}
...
listener "tcp" {
   address = "vault-1.moevideo.net:8200"
   cluster_address = "vault-1.moevideo.net:8201"
   tls_disable = true
}

seal "transit" {
   address            = "http://vault-0.moevideo.net:8200"
...
ui = true
disable_mlock = true
cluster_addr = "http://vault-1.moevideo.net:8201"
EOF
...
listener "tcp" {
   address = "vault-2.moevideo.net:8200"
   cluster_address = "vault-2.moevideo.net:8201"
   tls_disable = true
}
...
ui = true
disable_mlock = true
cluster_addr = "http://vault-2.moevideo.net:8201"
EOF
...
listener "tcp" {
   address = "vault-3.moevideo.net:8200"
   cluster_address = "vault-3.moevideo.net:8201"
   tls_disable = true
}
...
ui = true
disable_mlock = true
cluster_addr = "http://vault-3.moevideo.net:8201"
EOF
  printf "\n"
}
